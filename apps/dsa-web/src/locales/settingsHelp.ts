...
const settingsHelpZhCN: SettingsHelpMap = {
  ...
  'settings.system.market_review': {
    ...
    usage: 'MARKET_REVIEW_ENABLED 开启大盘分析；DAILY_MARKET_CONTEXT_ENABLED 默认开启，会把当日大盘摘要用于个股分析 Prompt 与保守护栏；MARKET_REVIEW_REGION 支持输入逗号分隔的市场子集（如 cn,us,jp 或 cn,us,jp,kr,tw），不传可回退为 cn；MARKET_REVIEW_COLOR_SCHEME 选择配色。',
    valueNotes: [
...
const settingsHelpEnUS: SettingsHelpMap = {
  ...
  'settings.system.market_review': {
    ...
    usage: 'MARKET_REVIEW_ENABLED toggles market review; DAILY_MARKET_CONTEXT_ENABLED is on by default and controls whether the daily market summary is injected into stock-analysis prompts and conservative guardrails; MARKET_REVIEW_REGION accepts a comma-separated region subset (for example cn,us,jp or cn,us,jp,kr,tw), and `both` keeps all supported markets; invalid or empty values fall back to `cn`; MARKET_REVIEW_COLOR_SCHEME selects colors.',
    valueNotes: [
      'cn covers A-shares, hk covers Hong Kong, us covers US stocks, jp covers Japan, kr covers Korea, tw covers Taiwan, and both covers all (cn,hk,us,jp,kr,tw).',
      'MARKET_REVIEW_REGION is free-text input; you can enter a comma-separated subset like cn,us or cn,us,jp,kr,tw.',
