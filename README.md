...
| 多市场数据聚合 | A股、港股、美股、ETF：行情、K 线、技术指标、资金流、筹码、新闻、公告和基本面；日股/韩股（`.T` / `.KS` / `.KQ`）：YFinance 日线与基础行情、技术指标可用，`capital_flow`、`dragon_tiger`、`boards` 与部分高阶区块会按市场边界降级为 `not_supported`；台股（`.TW` / `.TWO`）：在日韩基础能力之上，额外支持大盘复盘、三大法人买卖超、集保户股权集中度（TDCC 官方数据）与本地估算筹码分布（换手率衰减模型，非官方数据，仅供参考）（见 [市场支持边界](docs/market-support.md)） |
